...
<escalade:layout pageName="lengths">
	...
	<form:form modelAttribute="length" action="${fn:escapeXml(formUrl)}"
		method="get" class="form-horizontal" id="search-length-form">
	...
	<table id="lengthsTable" class="table table-striped">
		...
		<tbody>
			<c:forEach items="${selections}" var="length">
				<tr>
					<td><spring:url value="/lengths/{lengthId}" var="lengthUrl">
							<spring:param name="lengthId" value="${length.id} " />
						</spring:url> <a href="${fn:escapeXml(lengthUrl)}"> <c:out value="${length.name}" /></a>
					</td>								
					<td><c:out value="${length.comment} " /></td>
					<td><c:out value="${length.cotation} " /></td>
					<td><c:out value="${length.length_status} " /></td>
					<td><c:out value="${length.under_cotation} " /></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
